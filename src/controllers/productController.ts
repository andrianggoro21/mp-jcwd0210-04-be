...
const createProductController = async (req: Request, res: Response) => {
  try {
    const { productName, categoryId, price, stock, description, statusId } =
      req.body;
...
const getProductPaginationController = async (req: Request, res: Response) => {
  try {
    const { page, pageSize, productName, categoryId, alphaId, priceId } =
      req.query;
    ...
    const result = await productService.getProductPaginationService(
      parsedPage,
      parsedPageSize,
...
const filterProductAlphabetController = async (req: Request, res: Response) => {
  try {
    ...
    const result = await productService.filterProductAlphabetService(
      parsedFilterId
    );
    return res.status(200).json({
      message: "success",
...
const filterProductPriceController = async (req: Request, res: Response) => {
  try {
    const { filterId } = req.query;
    const parsedFilterId = parseInt(filterId as string, 10);
    const result = await productService.filterProductPriceService(
      parsedFilterId
    );
    return res.status(200).json({
      message: "success",
